# [NEXUS IDENTITY] ID: 5508572109453661285 | DATE: 2025-11-19
...
class ElasticsearchClient:
        ...
        for attempt in range(max_retries):
            try:
                if self.user and self.password:
                    ...
                    self.client = AsyncElasticsearch(
                        [f"http://{self.host}:{self.port}"], request_timeout=10.0
                    )

                # Test connection with timeout
                info = await asyncio.wait_for(self.client.info(), timeout=5.0)
            ...
            if not await self.client.indices.exists(index=self.INDEX_CODE):
                await self.client.indices.create(
                    ...
                    body={
                        ...
                        "mappings": {
                            "properties": {
                                "code": {"type": "text", "analyzer": "bsl_analyzer"},
                ...
                logger.info("Created index", extra={"index_name": self.INDEX_CODE})
    ...
    async def index_code(self, doc_id: str, code: str, metadata: Dict[str, Any]):
        """Index code for full-text search"""
        try:
            document = {"code": code, **metadata}

            await self.client.index(index=self.INDEX_CODE, id=doc_id, document=document)
            return True
    ...
    async def search_code(
        self, query: str, config_filter: Optional[str] = None, limit: int = 10
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if config_filter:
                must_queries.append({"term": {"configuration": config_filter}})
            ...
            response = await asyncio.wait_for(
                self.client.search(
                    ...
                    body={
                        ...
                        "size": limit,
                        "highlight": {"fields": {"code": {}, "description": {}}},
                    },
                ),
                timeout=10.0,
            )
    ...
    async def get_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if await self.client.indices.exists(index=self.INDEX_CODE):
                ...
                stats[self.INDEX_CODE] = {"document_count": count["count"]}
