# [NEXUS IDENTITY] ID: 5011754605728862271 | DATE: 2025-11-19
...
class EdgeFunction(ServerlessFunction):
    ...
    def __init__(self, handler: Callable, region: str = "global", timeout: int = 30):
    ...
    async def invoke(
        ...
        try:
            ...
            result = await asyncio.wait_for(
                self.handler(context, event), timeout=self.timeout
            )

            execution_time = (datetime.utcnow() - start_time).total_seconds() * 1000

            return FunctionResponse(
                status_code=200, body=result, execution_time_ms=execution_time
            )
...
class ServerlessRuntime:
    ...
    async def invoke(
        ...
        if context is None:
            context = FunctionContext(
                request_id=str(uuid4()), function_name=function_name
            )
    ...
    def get_metrics(
        self, function_name: Optional[str] = None, limit: int = 100
    ) -> List[Dict[str, Any]]:
        """Получение метрик"""
