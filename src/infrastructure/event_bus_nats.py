...
from src.infrastructure.event_bus import Event, EventBus, EventType, EventHandler
    ...
    def subscribe(self, event_type: EventType, handler: EventHandler) -> None:
        ...
        if self._js:
            ...
            sub = asyncio.create_task(
                self._js.subscribe(subject, cb=nats_handler)
            )
        else:
            # Подписка через обычный NATS
            sub = asyncio.create_task(
                self._nc.subscribe(subject, cb=nats_handler)
            )
