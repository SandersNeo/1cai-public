# [NEXUS IDENTITY] ID: -4318694224018103298 | DATE: 2025-11-19
    ...
    async def get_completions(
        ...
        if not isinstance(max_suggestions, int) or max_suggestions < 1:
            logger.warning(
                ...
                extra={
                    ...
                    "max_suggestions_type": type(max_suggestions).__name__,
                },
            )
            max_suggestions = 3
        ...
        try:
            ...
            logger.error(
                ...
                extra={"error_type": type(e).__name__, "code_length": len(code)},
                exc_info=True,
            )
...
async def get_completions(api_request: Request, request: CompletionRequest):
    ...
    try:
        ...
        current_line = max(
            1, min(request.current_line, 10000)
        )  # Clamp between 1 and 10000

        # Validate max_suggestions
        max_suggestions = max(
            1, min(request.max_suggestions, 20)
        ...
        raise HTTPException(
            status_code=500, detail="An error occurred while getting completions"
        )
...
async def generate_code(api_request: Request, request: GenerationRequest):
    ...
    try:
        ...
        code = await copilot_service.generate_code(
            prompt=prompt, code_type=code_type, timeout=10.0  # 10 seconds timeout
        )
