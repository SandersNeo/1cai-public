...
class ITSLibraryService:
    ...
    async def authenticate(self) -> bool:
        ...
        try:
            ...
            logger.info(
                "Авторизация через ITS", extra={"its_login_url": self.ITS_LOGIN_URL}
            )
            ...
            for login_url in self.ITS_LOGIN_URLS[
                1:
            ]:  # Пропускаем первый (уже использовали)
                try:
                    logger.info(
        ...
        try:
            ...
            for form in forms:
                ...
                username_input = (
                    ...
                    or form.find("input", {"name": "username"})
                    or form.find(
                        "input", {"name": re.compile(r"user|login|name", re.I)}
                    )
                    or form.find("input", {"id": re.compile(r"user|login|name", re.I)})
                )
                ...
                if username_input and password_input:
                    ...
                    response = self.session.post(
                        action_url, data=login_data, follow_redirects=True
                    )
                    ...
                    if response.status_code in [200, 302]:
                        ...
                        if any(
                            ...
                            or indicator in response_url_lower
                        ...
                        if (
                            "its.1c.ru" in response_url_str
                            and "login" not in response_url_str
                        ):
                            logger.info(
                                "Авторизация успешна (редирект на защищённую страницу)"
                            )
                            return True
    ...
    async def _try_direct_auth(self) -> bool:
        ...
        try:
            ...
            auth_endpoints = ["/user/auth/login", "/auth/login", "/login", "/db/login"]
                ...
                try:
                    ...
                    for auth_data in auth_data_variants:
                        try:
                            ...
                            response = self.session.post(
                                auth_url, data=auth_data, follow_redirects=True
                            )
                            ...
                            if response.status_code in [200, 302]:
                                ...
                                if self.session.cookies.get(
                                    "BITRIX_SM_LOGIN"
                                ) or self.session.cookies.get("auth"):
                                    self.authenticated = True
                                    logger.info(
                                        "Авторизация успешна",
                                ...
                                # Пробуем JSON
                                response = self.session.post(
                                    auth_url, json=auth_data, follow_redirects=True
                                )
                                if response.status_code in [200, 302]:
    ...
    async def get_configuration_documentation(self, config_name: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.debug("Получение страницы", extra={"page_url": page_url})
            ...
            logger.info("Извлечена документация", extra={"config_name": config_name})
    ...
    def _extract_description(self, soup: BeautifulSoup) -> str:
        ...
        desc_blocks = soup.find_all(
            ["p", "div"], class_=re.compile(r"description|about|info", re.I)
        )
    ...
    def _extract_modules(self, soup: BeautifulSoup) -> List[Dict[str, Any]]:
        ...
        module_sections = soup.find_all(
            ["div", "section"], class_=re.compile(r"module|module-info|content", re.I)
        )
        ...
        logger.info("Найдено модулей", extra={"modules_count": len(modules)})
        return modules

    def _find_module_context(
        ...
        try:
            ...
            return {"description": description.strip(), "code_examples": code_examples}
        except (httpx.HTTPError, httpx.RequestError, Exception):
            return None
    ...
    def _extract_best_practices(self, soup: BeautifulSoup) -> List[Dict[str, Any]]:
        ...
        for section in practice_sections:
            ...
            description = (
                ...
                if desc_elem
                else section.get_text(strip=True)[:500]
            )
        ...
        logger.info("Найдено best practices", extra={"practices_count": len(practices)})
        return practices
    ...
    def _extract_code_examples(self, soup: BeautifulSoup) -> List[Dict[str, Any]]:
            ...
            if code and len(code) > 30:
                ...
                if any(
                    ...
                    if code_hash not in seen_codes:
                        ...
                        description = (
                            prev_elem.get_text(strip=True)[:200] if prev_elem else ""
                        )

                        # Ищем заголовок секции
                        section_header = None
                        for header in code_block.find_all_previous(
                            ["h1", "h2", "h3", "h4"]
                        )[:1]:
        ...
        logger.info("Найдено примеров кода", extra={"examples_count": len(examples)})
        return examples
    ...
    def _extract_api_reference(self, soup: BeautifulSoup) -> List[Dict[str, Any]]:
        ...
        for table in tables:
            ...
            if any(
                ...
                for keyword in ["метод", "функция", "method", "function"]
            ):
                # Это таблица с API
                for row in rows[1:]:
    ...
    def _detect_category(self, text: str) -> str:
        ...
        if any(
            ...
            keyword in text_lower
            for keyword in ["безопасность", "security", "безопасный", "safe"]
        ):
            return "security"
        elif any(
            keyword in text_lower
            for keyword in ["ошибка", "error", "исключение", "exception"]
        ):
    ...
    async def load_all_configurations_docs(self) -> Dict[str, Any]:
        ...
        for config_name in configurations:
            logger.info("Загрузка документации", extra={"config_name": config_name})
