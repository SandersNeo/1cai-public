...
class ConfigurationKnowledgeBase:
        ...
        for config in self.SUPPORTED_CONFIGURATIONS:
            ...
            if config_file.exists():
                try:
                    ...
                    logger.info("Загружена база знаний", extra={"config": config})
    ...
    def get_configuration_info(self, config_name: str) -> Optional[Dict[str, Any]]:
        ...
        if config_key not in self.SUPPORTED_CONFIGURATIONS:
            logger.warning(
                "Неподдерживаемая конфигурация", extra={"config_name": config_name}
            )
            return None
    ...
    def _save_configuration(self, config_key: str) -> bool:
        ...
        try:
            ...
            logger.debug("Сохранена база знаний", extra={"config_key": config_key})
            return True
    ...
    def load_from_directory(self, directory_path: str) -> int:
        ...
        if not dir_path.exists() or not dir_path.is_dir():
            logger.error(
                "Директория не найдена", extra={"directory_path": str(directory_path)}
            )
            return 0
        ...
        for xml_file in dir_path.rglob("*.xml"):
            try:
                ...
                logger.info(
                    "Найден файл конфигурации", extra={"xml_file": str(xml_file)}
                )
                # Пока пропускаем
                continue
