# [NEXUS IDENTITY] ID: 4089778271909107920 | DATE: 2025-11-19
...
class ErrorMessages:
    ...
    VALIDATION_CODE_TOO_LONG = (
        "Code is too long. Maximum length: {max_length} characters."
    )
    ...
    @staticmethod
    def with_suggestion(
        error: str, suggestion: str, action_url: Optional[str] = None
    ) -> Dict:
        ...
        if action_url:
            response["action"] = {"label": "Learn more", "url": action_url}
...
def auth_error(message: str = None) -> Dict:
    ...
    return {
        ...
        "action": {"label": "Reset password", "url": "/reset-password"},
    }
...
def rate_limit_error(retry_after: int) -> Dict:
    ...
    return {
        "error": ErrorMessages.format(
            ErrorMessages.RATE_LIMIT_EXCEEDED, seconds=retry_after
        ),
