    ...
    async def _select_rl_action(self, state: RLState) -> RLAction:
        ...
        if np.random.random() < epsilon:
            ...
            if best_action:
                ...
                improvements = await self._generate_improvements(state.performance_metrics)
                return RLAction(
                    improvement_type="generate",
                    parameters={"improvements": improvements},
                    confidence=0.7
    ...
    async def _evaluate_multi_objective(
        ...
        for improvement in improvements:
            ...
            fitness = MultiObjectiveFitness(
                objectives={
                    OptimizationObjective.ACCURACY: improvement.expected_improvement.get("accuracy", 0.0),
                    OptimizationObjective.LATENCY: -improvement.expected_improvement.get("latency_ms", 0.0) / 1000.0,  # Отрицательное (меньше лучше)
                    OptimizationObjective.THROUGHPUT: improvement.expected_improvement.get("throughput", 0.0) / 100.0,
                    OptimizationObjective.USER_SATISFACTION: improvement.expected_improvement.get("user_satisfaction", 0.0)
                },
