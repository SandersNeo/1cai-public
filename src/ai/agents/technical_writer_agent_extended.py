# [NEXUS IDENTITY] ID: 3807306823955458496 | DATE: 2025-11-19
...
class APIDocumentationGenerator:
    ...
    async def generate_api_docs(
        self, code: str, module_type: str = "http_service"
    ) -> Dict[str, Any]:
        """
        ...
        """
        logger.info("Generating API documentation", extra={"module_type": module_type})
    ...
    def _extract_endpoints(self, code: str) -> List[Dict]:
        ...
        for match in matches:
            ...
            http_method = "GET"
            if any(
                kw in function_name.lower()
                for kw in ["создать", "добавить", "create", "add"]
            ):
                http_method = "POST"
            elif any(
                kw in function_name.lower() for kw in ["обновить", "изменить", "update"]
            ):
                http_method = "PUT"
    ...
    def _generate_examples(self, endpoints: List[Dict]) -> List[Dict]:
        ...
        for endpoint in endpoints:
            ...
            if endpoint["parameters"]:
                params = "&".join(
                    [f"{p['name']}=value" for p in endpoint["parameters"]]
                )
                curl += f"?{params}"
            ...
            examples.append(
                {
                    ...
                    "request": {
                        ...
                        "url": endpoint["path"],
                        "parameters": {
                            p["name"]: "example_value" for p in endpoint["parameters"]
                        },
                    },
                    "response": {
                        "status": 200,
...
class UserGuideGenerator:
    ...
    async def generate_user_guide(
        self, feature: str, target_audience: str = "end_user"
    ) -> Dict[str, Any]:
        """
        ...
        """
        logger.info("Generating user guide", extra={"feature": feature})
    ...
    def _generate_sections(self, feature: str, audience: str) -> List[Dict]:
        ...
        sections = [
            {"title": "Обзор", "content": f"Функция '{feature}' предназначена для..."},
            {
                "title": "Начало работы",
        ...
        if audience == "end_user":
            sections.extend(
                [
                    ...
                    {"title": "Часто используемые сценарии", "content": "..."},
                ]
            )
            ...
            sections.extend(
                [
                    {"title": "Конфигурация", "content": "..."},
                    {"title": "Мониторинг и обслуживание", "content": "..."},
                ]
            )
    ...
    def _assemble_guide(
        self, feature: str, sections: List[Dict], faq: List[Dict], audience: str
    ) -> str:
...
class ReleaseNotesGenerator:
    ...
    async def generate_release_notes(
        self, git_commits: List[Dict], version: str
    ) -> str:
        """
        ...
        """
        logger.info("Generating release notes", extra={"version": version})
        ...
        for commit in git_commits:
            ...
            if msg.startswith("feat:") or msg.startswith("feature:"):
                features.append(
                    msg.replace("feat:", "").replace("feature:", "").strip()
                )
...
class CodeDocumentationGenerator:
    ...
    async def document_function(self, function_code: str, language: str = "bsl") -> str:
        """
        ...
        if language == "bsl":
            ...
            return (
                "// Documentation generation for this language is not yet implemented"
            )
...
"""
        ...
        documented_code = re.sub(
            r"(Функция\s+" + func_name + r")", doc + r"\1", code, count=1
        )
...
class TechnicalWriterAgentExtended:
    ...
    async def generate_user_guide(
        self, feature: str, audience: str = "end_user"
    ) -> Dict[str, Any]:
        """Генерация user guide"""
        return await self.guide_generator.generate_user_guide(feature, audience)

    async def generate_release_notes(self, commits: List[Dict], version: str) -> str:
        """Генерация release notes"""
        return await self.release_notes_generator.generate_release_notes(
            commits, version
        )

    async def document_code(self, code: str, language: str = "bsl") -> str:
