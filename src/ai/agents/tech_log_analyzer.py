...
class TechLogAnalyzer:
    ...
    async def parse_tech_log(
        ...
        if time_period:
            events = [
                e for e in events if time_period[0] <= e.timestamp <= time_period[1]
            ]
    ...
    def _determine_severity(
        self, event_type: str, duration_ms: int, error: Optional[str]
    ) -> str:
        ...
        if event_type == "EXCP":
            if error and any(
                kw in error.lower() for kw in ["deadlock", "timeout", "connection"]
            ):
                return "critical"
            return "high"
    ...
    async def analyze_performance(self, log_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        for event in dbmssql_events:
            if event.sql:
                ...
                if sql_key not in sql_stats:
                    sql_stats[sql_key] = {"sql": event.sql, "durations": [], "count": 0}
