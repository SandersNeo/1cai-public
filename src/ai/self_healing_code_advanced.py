...
class AdvancedSelfHealingCode(SelfHealingCode):
    ...
    def __init__(
        ...
        self._fix_history: List[Tuple[CodeError, CodeFix, bool]] = []  # (error, fix, success)
    ...
    async def handle_error(
        ...
        if self.learn_from_history:
            ...
            if similar_errors:
                historical_fix = await self._try_historical_fix(code_error, similar_errors)
                if historical_fix:
    ...
    def _find_similar_errors(self, error: CodeError) -> List[CodeError]:
        ...
        similar.sort(
            key=lambda e: self._calculate_error_similarity(error, e),
            reverse=True
        )
    ...
    def get_pattern_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "patterns_by_success": {
                ...
                "medium": len([p for p in self._fix_patterns if 0.5 < p.success_rate() <= 0.8]),
