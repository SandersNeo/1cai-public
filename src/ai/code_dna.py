...
class CodeDNAEngine:
    ...
    def crossover(self, dna1: CodeDNA, dna2: CodeDNA) -> CodeDNA:
        ...
        top_genes_1 = sorted(dna1.genes, key=lambda g: g.fitness, reverse=True)[:len(dna1.genes)//2]
        child_genes.extend(top_genes_1)
        
        # Гены от второго родителя (лучшие по fitness)
        top_genes_2 = sorted(dna2.genes, key=lambda g: g.fitness, reverse=True)[:len(dna2.genes)//2]
