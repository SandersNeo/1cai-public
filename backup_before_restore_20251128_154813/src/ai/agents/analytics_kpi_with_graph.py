...
class KPIGeneratorWithGraph:
    ...
    async def generate_kpis_from_graph(
        ...
        *,
        ...
        if not self.backend:
            logger.warning(
                "Graph backend not available, returning template KPIs")
    ...
    async def _build_technical_kpis(
        ...
        incident_rate = (len(all_incidents) /
                         len(all_modules) if len(all_modules) > 0 else 0)
        ...
        kpis.append({"id": "kpi_change_failure_rate",
