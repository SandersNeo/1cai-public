...
class IntelligentCache:
    ...
    def _generate_key(
        ...
        Returns:
            Хэш-ключ для кэша
        """
        # Нормализовать запрос (убрать лишние пробелы, привести к нижнему
        # регистру)
    ...
    def invalidate_by_query_type(self, query_type: str) -> int:
        ...
        keys_to_remove = [
            key for key,
            entry in self._cache.items() if entry.query_type == query_type]
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        now = datetime.utcnow()
        ages = [(now - entry.created_at).total_seconds()
