...
class AdvancedCodeDNAEngine(CodeDNAEngine):
    ...
    def mutate_advanced(
            self,
            dna: CodeDNA,
            mutation_type: MutationType) -> CodeDNA:
        """Расширенная мутация"""
    ...
    def _structural_mutation(self, dna: CodeDNA) -> CodeDNA:
        ...
        gene = secrets.choice(mutated.genes)
    ...
    def _inversion_mutation(self, dna: CodeDNA) -> CodeDNA:
        ...
        if len(mutated.genes) > 1:
            start = secrets.randbelow(0, len(mutated.genes) - 1)
            end = secrets.randbelow(start + 1, len(mutated.genes))
    ...
    def _deletion_mutation(self, dna: CodeDNA) -> CodeDNA:
        ...
        if len(mutated.genes) > 1:
            ...
            deleted = mutated.genes.pop(
                secrets.randbelow(0, len(mutated.genes) - 1))
    ...
    def _single_point_crossover(self, dna1: CodeDNA, dna2: CodeDNA) -> CodeDNA:
        ...
        point = secrets.randbelow(1, min(len(dna1.genes), len(dna2.genes)) - 1)
    ...
    def _two_point_crossover(self, dna1: CodeDNA, dna2: CodeDNA) -> CodeDNA:
        ...
        point1 = secrets.randbelow(1, max_len - 1)
        point2 = secrets.randbelow(point1 + 1, max_len)

        child_genes = (dna1.genes[:point1] +
                       dna2.genes[point1:point2] +
                       dna1.genes[point2:])
    ...
    async def evolve_with_islands(
        ...
        for generation in range(generations):
            logger.info("Island evolution generation {generation + 1}/%s")
            ...
            for island in self._islands:
                ...
                while len(island.population) < self._population_size:
                    parent1 = secrets.choice(island.population)
                    parent2 = secrets.choice(island.population)

                    if random.random() < self._crossover_rate:
                        child = self.crossover_advanced(parent1, parent2)
                    else:
                        child = self.mutate_advanced(
                            parent1, MutationType.STRUCTURAL)
    ...
    async def _migrate_between_islands(self) -> None:
        ...
        for i, island in enumerate(self._islands):
            if random.random() < island.migration_rate:
                ...
                migrants = island.population[: int(
                    len(island.population) * 0.1)]
