...
from src.ml.continual_learning.conversational_memory import \
...
class NestedAssistant:
    ...
    def __init__(
            ...
            name: str,
            system_prompt: str,
            base_assistant: Optional[Any] = None):
        """
        ...
        logger.info("Created NestedAssistant: %s (%s)")
    ...
    def start_session(self, session_id: str):
        ...
        logger.info("Started session %s for {self.name}")
    ...
    def process_message(self, user_message: str,
        ...
        logger.debug(
            ...
            extra={
                "assistant": self.name,
                "context_messages": len(relevant_context)})
    ...
    def provide_feedback(
        ...
        if response_id not in self.response_history:
            logger.warning("Unknown response_id: %s")
            return

        self.stats["total_feedback"] += 1

        # Update average rating
        self.stats["avg_rating"] = self.stats["avg_rating"] * \
            0.9 + rating * 0.1
