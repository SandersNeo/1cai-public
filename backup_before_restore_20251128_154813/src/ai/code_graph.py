...
class EdgeKind(str, Enum):
    ...
    BSL_CALLS = (
        # Вызов функции/процедуры (более специфичный чем DEPENDS_ON)
        "BSL_CALLS"
    )
    # Использование объекта метаданных в коде
    BSL_USES_METADATA = "BSL_USES_METADATA"
    # Работа с БД
    BSL_READS_TABLE = "BSL_READS_TABLE"  # Чтение таблицы БД (SELECT)
    # Запись в таблицу БД (INSERT/UPDATE/DELETE)
    BSL_WRITES_TABLE = "BSL_WRITES_TABLE"
    ...
    # Наследование (объект расширяет другой объект)
    BSL_EXTENDS = "BSL_EXTENDS"
