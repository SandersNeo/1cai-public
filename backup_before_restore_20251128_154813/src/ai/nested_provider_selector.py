...
class NestedProviderSelector:
    ...
    def select_provider_adaptive(
        ...
        decision = {
            "query": query,
            "query_type": (
                query_type.value if hasattr(
                    query_type,
                    "value") else str(query_type)),
            ...
            "timestamp": time.time(),
            "similar_count": sum(
                len(v) for v in similar_queries.values()),
        }
    ...
    def learn_from_feedback(
        self, query_id: str, success: bool, metrics: Dict[str, Any]
    ):
        """
        Continual learning from feedback
        ...
        if surprise > 0.7:  # High surprise
            ...
            self.query_memory.update_level(
                "session", query_id, feedback_data, surprise)
            self.query_memory.update_level(
                "daily", query_id, feedback_data, surprise)
    ...
    def _compute_feedback_surprise(
        self, success: bool, metrics: Dict[str, Any]
    ) -> float:
        """
        ...
        success_rate = (
            self.stats["success_count"] / self.stats["total_feedback"]
