...
from .config import (CHAIRMAN_MODEL, COUNCIL_MODELS, COUNCIL_TIMEOUT_SECONDS,
                     MAX_COUNCIL_SIZE, MIN_COUNCIL_SIZE)
...
class CouncilOrchestrator:
    ...
    async def process_query(
            ...
            config: Optional[CouncilConfig] = None) -> CouncilResponse:
        """
        ...
        logger.info("Starting council query with {len(config.models)} models")
    ...
    def _validate_config(self, config: CouncilConfig):
        ...
        if config.chairman not in config.models:
            logger.warning(
                f"Chairman {config.chairman} not in council models, "
                f"will use separate provider")
