...
class CodeDNAEngine:
    ...
    def encode_code(self, code: str, language: str = "bsl") -> List[CodeGene]:
        ...
        self.logger.info("Encoded code into {len(genes)} genes")
        return genes
    ...
    def _encode_bsl(self, code: str) -> List[CodeGene]:
        ...
        for line in code.split("\n"):
            ...
            elif stripped.startswith("КонецФункции") or stripped.startswith(
                "КонецПроцедуры"
            ):
    ...
    def evaluate_fitness(
        self, genes: List[CodeGene], target_metrics: Dict[str, Any]
    ) -> float:
        """
        ...
        self.logger.info("Average fitness: {avg_fitness:.2f}")
        return avg_fitness

    def _evaluate_gene_fitness(
        self, gene: CodeGene, target_metrics: Dict[str, Any]
    ) -> float:
    ...
    async def evolve_code(
        self, code: str, target_metrics: Dict[str, Any], generations: int = 5
    ) -> Dict[str, Any]:
        """
        Evolve code toward target metrics
        ...
        for gen in range(generations):
            ...
            if new_fitness > best_fitness:
                ...
                self.logger.info("Gen {gen+1}: Improved fitness to {best_fitness:.2f}")
            else:
                self.logger.info("Gen {gen+1}: No improvement ({new_fitness:.2f})")
    ...
    def save_genome(self, genome_id: str, genes: List[CodeGene]):
        ...
        self.logger.info("Saved genome %s")
