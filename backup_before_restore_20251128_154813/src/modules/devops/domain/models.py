...
class PipelineOptimization(BaseModel):
    ...
    expected_speedup_percent: int = Field(
        ..., ge=0, le=100, description="Ожидаемое ускорение в %"
    )
...
class LogAnalysisResult(BaseModel):
    ...
    timestamp: str = Field(
        default_factory=lambda: datetime.now().isoformat(), description="Время анализа"
    )
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "errors_by_category": {"memory": 15, "network": 20, "database": 10},
                "anomalies": [],
                "patterns": [],
...
class CostOptimizationResult(BaseModel):
    ...
    current_cost_month: float = Field(
        ..., ge=0, description="Текущая стоимость в месяц"
    )
    optimized_cost_month: float = Field(
        ..., ge=0, description="Оптимизированная стоимость"
    )
...
class UsageMetrics(BaseModel):
    ...
    memory_avg: float = Field(
        ..., ge=0, le=100, description="Средняя загрузка памяти в %"
    )
