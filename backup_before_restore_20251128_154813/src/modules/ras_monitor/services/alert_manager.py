...
class AlertManager:
    ...
    async def generate_alerts(
        ...
        for resource in resources:
            ...
            threshold = self.monitoring_repository.get_threshold(threshold_key)

            # Check if threshold exceeded
            if resource.usage_percent > threshold:
                severity = self._determine_severity(resource.usage_percent)

                alert = ResourceAlert(
                    resource_type=resource.resource_type,
                    severity=severity,
                    message=self._generate_message(
                        resource.resource_type, resource.usage_percent
                    ),
    ...
    def _generate_message(
        ...
        """Генерация сообщения"""
        return f"High {resource_type.value} usage detected: " f"{usage_percent:.1f}%"
    ...
    async def prioritize_alerts(
        ...
        return sorted(
            alerts, key=lambda a: (severity_order.get(a.severity, 3), -a.current_value)
        )

    async def should_notify(self, alert: ResourceAlert) -> bool:
        """
