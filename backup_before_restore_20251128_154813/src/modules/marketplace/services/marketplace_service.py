...
MAX_ARTIFACT_SIZE_BYTES = int(
    os.getenv(
        "MARKETPLACE_MAX_ARTIFACT_SIZE_MB",
        "25")) * 1024 * 1024
...
class MarketplaceService:
    ...
    async def submit_plugin(
            self, plugin_data: Dict[str, Any], user: CurrentUser) -> Dict[str, Any]:
        """Submit a new plugin."""
    ...
    async def update_plugin(
        ...
        if not self._check_authorization(user, plugin):
            raise PermissionError(
                "You don't have permission to update this plugin")

        if "version" in update_data and update_data["version"] != plugin.get(
                "version"):
    ...
    async def delete_plugin(self, plugin_id: str,
                            user: CurrentUser) -> Optional[Dict[str, Any]]:
        """Soft delete plugin."""
        plugin = await self.repo.get_plugin(plugin_id)
        if not plugin:
            return None
        ...
        if not self._check_authorization(user, plugin):
            raise PermissionError(
                "You don't have permission to delete this plugin")
        ...
        audit_logger.log_action(
            ...
            target=plugin_id,
            metadata={
                "status": removed.get("status") if removed else "removed"},
        )
        return removed

    async def record_install(self, plugin_id: str,
    ...
    async def create_review(self,
        ...
        if not await self.repo.user_has_installed(plugin_id, user.user_id):
            raise PermissionError(
                "You must install the plugin before leaving a review")
    ...
    async def add_favorite(self, plugin_id: str, user_id: str) -> None:
        ...
        logger.info(
            "Plugin %s added to favorites by user %s",
            plugin_id,
            user_id)
    ...
    async def remove_favorite(self, plugin_id: str, user_id: str) -> None:
        ...
        logger.info(
            "Plugin %s removed from favorites by user %s",
            plugin_id,
            user_id)
    ...
    def _check_authorization(self, user: CurrentUser,
                             plugin: Dict[str, Any]) -> bool:
