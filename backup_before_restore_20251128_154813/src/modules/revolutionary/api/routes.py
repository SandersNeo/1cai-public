...
async def health_check(
    ...
    return {
        ...
        "components": [
            {"name": comp.name, "status": comp.status, "enabled": comp.enabled}
...
@router.post(
    "/evolve", summary="Trigger evolution cycle", response_model=Dict[str, Any]
)
...
@router.post("/heal", summary="Trigger code healing", response_model=Dict[str, Any])
async def trigger_healing(
    code: str, orchestrator: RevolutionaryOrchestrator = Depends(get_orchestrator)
) -> Dict[str, Any]:
    """
...
@router.get("/metrics", summary="Get Prometheus metrics", response_model=Dict[str, Any])
