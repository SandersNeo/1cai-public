...
async def get_current_user(
        request: Request,
        token: Optional[str] = Depends(oauth2_scheme)) -> CurrentUser:
...
def require_roles(*roles: str):
    async def dependency(user: CurrentUser = Depends(
...
async def get_current_user_id(
        current_user: CurrentUser = Depends(get_current_user)) -> int:
    """
    ...
    # Предположим пока что мы можем использовать hash или int conversion если
    # это число.
