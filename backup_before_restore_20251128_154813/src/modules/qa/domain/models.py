...
class TestCase(BaseModel):
    ...
    parameters: List[TestParameter] = Field(
        default_factory=list, description="Параметры теста"
    )
...
class TestGenerationResult(BaseModel):
    ...
    positive_tests: List[TestCase] = Field(
        default_factory=list, description="Позитивные тесты"
    )
    negative_tests: List[TestCase] = Field(
        default_factory=list, description="Негативные тесты"
    )
    edge_case_tests: List[TestCase] = Field(
        default_factory=list, description="Граничные случаи"
    )
    coverage_estimate: str = Field(..., description="Оценка покрытия (например, '85%')")
    complexity: int = Field(
        ..., ge=1, le=10, description="Цикломатическая сложность (1-10)"
    )
    timestamp: str = Field(
...
class CoverageReport(BaseModel):
    ...
    total_coverage: float = Field(
        ..., ge=0.0, le=100.0, description="Общее покрытие (%)"
    )
    line_coverage: float = Field(
        ..., ge=0.0, le=100.0, description="Покрытие строк (%)"
    )
    branch_coverage: float = Field(
        ..., ge=0.0, le=100.0, description="Покрытие ветвлений (%)"
    )
    function_coverage: float = Field(
        ..., ge=0.0, le=100.0, description="Покрытие функций (%)"
    )
...
class TestTemplate(BaseModel):
    ...
    parameters: List[str] = Field(default_factory=list, description="Параметры шаблона")
