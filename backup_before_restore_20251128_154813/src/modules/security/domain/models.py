...
class VulnerabilityScanResult(BaseModel):
    ...
    vulnerabilities: List[Vulnerability] = Field(
        default_factory=list, description="Обнаруженные уязвимости"
    )
    risk_score: float = Field(..., ge=0.0, le=100.0, description="Оценка риска (0-100)")
    severity_breakdown: Dict[str, int] = Field(
        default_factory=dict, description="Распределение по серьезности"
    )
    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
                "vulnerabilities": [],
                "risk_score": 45.5,
                "severity_breakdown": {"critical": 1, "high": 2, "medium": 3, "low": 5},
...
class DependencyVulnerability(BaseModel):
    ...
    severity: Severity = Field(..., description="Серьезность")
    description: str = Field(..., description="Описание уязвимости")
    fixed_version: Optional[str] = Field(None, description="Версия с исправлением")
...
class DependencyAuditResult(BaseModel):
    ...
    vulnerable_dependencies: List[DependencyVulnerability] = Field(
        default_factory=list, description="Уязвимые зависимости"
    )
...
class SecretDetectionResult(BaseModel):
    ...
    secrets_found: List[Secret] = Field(
        default_factory=list, description="Обнаруженные секреты"
    )
    total_count: int = Field(..., ge=0, description="Всего найдено")
    high_confidence_count: int = Field(
        ..., ge=0, description="С высокой уверенностью (>0.8)"
    )
    timestamp: str = Field(
        default_factory=lambda: datetime.now().isoformat(),
...
class ComplianceReport(BaseModel):
    ...
    issues: List[ComplianceIssue] = Field(
        default_factory=list, description="Обнаруженные проблемы"
    )
    compliance_score: float = Field(
        ..., ge=0.0, le=100.0, description="Оценка compliance (0-100)"
    )
    timestamp: str = Field(
        default_factory=lambda: datetime.now().isoformat(),
