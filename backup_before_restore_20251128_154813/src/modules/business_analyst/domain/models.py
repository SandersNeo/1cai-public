...
class UserStory(BaseModel):
    ...
    acceptance_criteria: List[str] = Field(
        default_factory=list, description="Критерии приемки"
    )
...
class Requirement(BaseModel):
    ...
    confidence: float = Field(
        ..., ge=0.0, le=1.0, description="Уверенность в извлечении (0-1)"
    )
    stakeholders: List[str] = Field(
        default_factory=list, description="Заинтересованные стороны"
    )
    acceptance_criteria: List[str] = Field(
        default_factory=list, description="Критерии приемки"
    )
...
class RequirementExtractionResult(BaseModel):
    ...
    functional_requirements: List[Requirement] = Field(
        default_factory=list, description="Функциональные требования"
    )
    non_functional_requirements: List[Requirement] = Field(
        default_factory=list, description="Нефункциональные требования"
    )
    constraints: List[Requirement] = Field(
        default_factory=list, description="Ограничения"
    )
    stakeholders: List[str] = Field(
        default_factory=list, description="Заинтересованные стороны"
    )
    ...
    summary: Dict[str, Any] = Field(
        default_factory=dict, description="Сводка по требованиям"
    )
    timestamp: str = Field(
...
class BPMNDiagram(BaseModel):
    ...
    diagram_svg: Optional[str] = Field(
        None, description="SVG представление (опционально)"
    )
    mermaid: str = Field(..., description="Mermaid diagram")
    actors: List[str] = Field(default_factory=list, description="Участники процесса")
    activities: List[str] = Field(default_factory=list, description="Активности")
...
class Gap(BaseModel):
    ...
    priority: float = Field(..., ge=0.0, le=10.0, description="Приоритет (0-10)")
...
class RoadmapItem(BaseModel):
    ...
    dependencies: List[str] = Field(
        default_factory=list, description="Зависимости от других фаз"
    )
...
class GapAnalysisResult(BaseModel):
    ...
    roadmap: List[RoadmapItem] = Field(
        default_factory=list, description="Дорожная карта"
    )
...
class CoverageSummary(BaseModel):
    ...
    coverage_percent: float = Field(
        ..., ge=0.0, le=100.0, description="Процент покрытия"
    )
...
class TraceabilityMatrix(BaseModel):
    ...
    matrix: List[TraceabilityItem] = Field(default_factory=list, description="Матрица")
    coverage_summary: CoverageSummary = Field(..., description="Сводка по покрытию")
    timestamp: str = Field(
        default_factory=lambda: datetime.now().isoformat(),
        description="Timestamp анализа",
    )
