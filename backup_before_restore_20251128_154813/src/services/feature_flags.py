# [NEXUS IDENTITY] ID: 3143134187181721926 | DATE: 2025-11-19
...
class FeatureFlagService:
    ...
    def register(self, flag: FeatureFlag):
        ...
        logger.info(
            ...
            extra={"flag_key": flag.key, "state": flag.state.value, "name": flag.name},
        )
    ...
    def is_enabled(
        ...
        if flag_key not in self.flags:
            logger.warning("Unknown feature flag", extra={"flag_key": flag_key})
            return False
    ...
    def get_all_flags(self, user_id: Optional[str] = None) -> Dict[str, bool]:
        ...
        return {key: self.is_enabled(key, user_id=user_id) for key in self.flags.keys()}
    ...
    def update_flag(
        ...
        if flag_key not in self.flags:
            logger.warning("Unknown flag in update_flag", extra={"flag_key": flag_key})
            ...
            if not isinstance(percentage, int) or percentage < 0 or percentage > 100:
                logger.warning(
                    ...
                    extra={
                        ...
                        "percentage_type": type(percentage).__name__,
                    },
                )
