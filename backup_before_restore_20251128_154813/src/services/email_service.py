...
class EmailService:
    ...
    def __init__(
        ...
        self.min_alert_interval = int(
            os.getenv("EMAIL_RATE_LIMIT_SECONDS", "3600")
        )  # 1 hour default
        ...
        if not all([self.smtp_user, self.smtp_password]):
            ...
            self.enabled = True
            logger.info("Email service initialized: {self.smtp_host}:{self.smtp_port}")
    ...
    def send_email(
        ...
        try:
            ...
            logger.error("Failed to send email: %s", exc_info=True)
            return False
    ...
    def send_drift_alert(self, drift_models: List[Dict], to_emails: List[str]) -> bool:
        ...
        if not self._check_rate_limit(alert_key):
            logger.info(
                f"Drift alert skipped due to rate limiting "
                f"(min interval: {self.min_alert_interval}s)"
            )
            return False
