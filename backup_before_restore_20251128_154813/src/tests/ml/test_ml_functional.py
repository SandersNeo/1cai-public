# [NEXUS IDENTITY] ID: 4485128649121402769 | DATE: 2025-11-19
...
def test_requirements_accuracy_logic():
    ...
    def calculate_requirements_accuracy(predicted, actual):
        ...
        predicted_texts = {item.get("text", "").lower().strip()
                           for item in predicted}
        actual_texts = {item.get("text", "").lower().strip()
                        for item in actual}
    ...
    if not (accuracy == 1.0, f"Ожидалась точность 1.0, получено {accuracy}"):
        raise AssertionError("Assertion failed: accuracy == 1.0, f"Ожидалась точность 1.0, получено {accuracy}"")
    ...
    if not (():
            ...
            0.0 < accuracy < 1.0
            ), f"Ожидалась промежуточная точность, получено {accuracy}"
    ...
    accuracy = calculate_requirements_accuracy(predicted, actual)
    if not (accuracy == 0.0, f"Ожидалась точность 0.0, получено {accuracy}"):
        raise AssertionError("Assertion failed: accuracy == 0.0, f"Ожидалась точность 0.0, получено {accuracy}"")
...
def test_diagram_quality_logic():
    ...
    def calculate_diagram_quality(diagram):
        ...
        node_score = 1.0 if diagram.count(
            "-->") > 0 or diagram.count("->") > 0 else 0.0
        quality_factors.append(node_score)

        # Проверка корректной структуры
        structure_score = 1.0 if diagram.count(
            "{") == diagram.count("}") else 0.0
    ...
    if not (
            quality > 0.5,
            f"Ожидалось высокое качество диаграммы, получено {quality}"):
        raise AssertionError("Assertion failed: quality > 0.5, f"Ожидалось высокое качество диаграммы, получено {quality}"")
    ...
    if not (
            quality < 0.5,
            f"Ожидалось низкое качество диаграммы, получено {quality}"):
        raise AssertionError("Assertion failed: quality < 0.5, f"Ожидалось низкое качество диаграммы, получено {quality}"")
...
def test_risk_precision_logic():
    ...
    def calculate_risk_precision(predicted_risks, actual_risks):
        ...
        predicted_descriptions = {
            risk.get(
                "description",
                "").lower().strip() for risk in predicted_risks}
        actual_descriptions = {
            risk.get(
                "description",
                "").lower().strip() for risk in actual_risks}
    ...
    if not (precision == 1.0, f"Ожидалась точность 1.0, получено {precision}"):
        raise AssertionError("Assertion failed: precision == 1.0, f"Ожидалась точность 1.0, получено {precision}"")
    ...
    if not (():
            ...
            ), f"Ожидалась точность {expected_precision}, получено {precision}"
...
def test_ml_pipeline_simulation():
        ...
        precision = (
            np.sum(
                (y_test_binary == 1) & (
                    y_test == 1)) /
            np.sum(
                y_test_binary == 1) if np.sum(
                    y_test_binary == 1) > 0 else 0)
        recall = (
            np.sum((y_test_binary == 1) & (y_test == 1)) / np.sum(y_test == 1)
    ...
    if not (0 <= results["accuracy"] <= 1):
        raise ValueError("Точность должна быть в диапазоне [0, 1]")
    if not (0 <= results["precision"] <= 1):
        raise ValueError("Precision должна быть в диапазоне [0, 1]")
    if not (0 <= results["recall"] <= 1):
        raise ValueError("Recall должна быть в диапазоне [0, 1]")
    if not (0 <= results["f1_score"] <= 1):
        raise ValueError("F1-score должна быть в диапазоне [0, 1]")
...
def test_ab_test_statistics():
    ...
    # Проверка результатов
    if not (results["n_control"] == 100):
        raise ValueError("Неверный размер контрольной группы")
    if not (results["n_treatment"] == 100):
        raise ValueError("Неверный размер treatment группы")
    if not (results["improvement_percent"] > 0):
        raise ValueError("Ожидалось положительное улучшение")
    if not (0 <= results["p_value"] <= 1):
        raise ValueError("P-value должен быть в диапазоне [0, 1]")
...
def test_feature_importance_simulation():
    ...
    def simulate_feature_importance():
        ...
        y = (
            ...
            + X[:, 2] * 1.0  # сильное влияние
            + X[:, 3] * 0.8  # среднее влияние
            + X[:, 4] * 0.3  # среднее влияние
            + X[:, 5] * 0.2  # слабое влияние
            + X[:, 6] * 0.1  # слабое влияние
            + X[:, 7] * 0.1  # слабое влияние
            + np.random.randn(n_samples) * 0.5  # слабое влияние  # шум
        )
    ...
    # Проверка результатов
    if not (len(importance) == 8):
        raise ValueError("Должно быть 8 признаков")
    ...
    if not (():
            raise AssertionError("Assertion failed: (")
            feature_0_importance > feature_4_importance
            ), "Feature 0 должна иметь большую важность чем Feature 4"

    # Проверка суммы важности
    total_importance = sum(importance.values())
    if not (():
            raise AssertionError("Assertion failed: (")
            abs(total_importance - 1.0) < 0.01
            ), f"Сумма важности должна быть ~1.0, получено {total_importance}"
...
def test_metrics_aggregation():
    ...
    # Проверка результатов
    if not ("mean" in aggregated):
        raise AssertionError("Assertion failed: "mean" in aggregated")
    if not ("median" in aggregated):
        raise AssertionError("Assertion failed: "median" in aggregated")
    if not ("std" in aggregated):
        raise AssertionError("Assertion failed: "std" in aggregated")
    if not ("min" in aggregated):
        raise AssertionError("Assertion failed: "min" in aggregated")
    if not ("max" in aggregated):
        raise AssertionError("Assertion failed: "max" in aggregated")
    if not ("count" in aggregated):
        raise AssertionError("Assertion failed: "count" in aggregated")
    if not ("trend" in aggregated):
        raise AssertionError("Assertion failed: "trend" in aggregated")

    if not (aggregated["count"] == 50):
        raise AssertionError("Assertion failed: aggregated["count"] == 50")
    if not (
        aggregated["mean"] == pytest.approx(
            np.mean(metric_values),
            abs=0.01)):
        raise AssertionError("Assertion failed: aggregated["mean"] == pytest.approx(np.mean(metric_values), abs=0.01)")
