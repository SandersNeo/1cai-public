...
| **Observability & Monitoring** | RED/USE метрики, логи, трассировки, SLO/Error Budgets, alert runbooks | `github-monitor.yml`, `dora-metrics.yml`, `/metrics` (Prometheus Instrumentator), локальный стек (`make observability-up`), `docs/observability/SLO.md`, `docs/runbooks/alert_slo_runbook.md`; TODO: реальный alert канал | Добавить сбор SLI (Prometheus exporter/Logs), завести alert-канал |
